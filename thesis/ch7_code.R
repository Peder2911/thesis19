...
holdPred <- read.csv('data/holdout_preds.csv', stringsAsFactors = FALSE)
...
modelGrid <- reshape::melt(modelConfigs, id.vars = 'name') %>% 
   ...
   geom_point(size = 5, color = 'springgreen3') + 
...
ggplot(resultSum, aes(x = name)) + 
   geom_point(aes(y = f1mean), size = 10) + 
   geom_segment(aes(xend = name,y = f1mean-f1sd, yend = f1mean + f1sd), size = 3) + 
   coord_flip() +
   scale_x_discrete(limits = rev(levels(factor(summa$name)))) 
...
inline <- ggplot(bestFamRes, aes(x = fallout, y = recall, color = name)) +
   ...
   labs(x = '',
        y = '')+
   scale_color_manual(values = c('springgreen3','firebrick3','steelblue3'))
...
xtable(confusionMatrix, caption = "Holdout confusion matrix")
